...
YOLO_DIR = r"D:\RAJU\rs\IssacSim\rockynotes_yolo_880"
...
def process_split(data_list, split_name):
    for img_path, bbox_npy, bbox_json, inst_img, inst_json, folder, frame_suffix in data_list:
        ...
        with open(dest_bb_label, 'w') as out_f:
            for box in bboxes:
                class_name = bb_labels.get(str(box['semanticId']), {}).get('class', 'unknown')
                if class_name in ['unknown', 'UNLABELLED', 'unlabelled', 'background']: continue
        ...
        with open(dest_seg_label, 'w') as out_f:
            ...
            for uid in unique_ids:
                ...
                class_name = inst_labels.get(str(uid), {}).get('class', 'unknown')
                if class_name in ['unknown', 'UNLABELLED', 'unlabelled', 'background']: continue
...
print("\n" + "="*70)
print(f"✅ DUAL DATASET GENERATION COMPLETE! ✅")
print(f"-> Object Detection ready at: {BB_DIR}")
print(f"-> Instance Segmentation ready at: {SEG_DIR}")
print(f"Detected Classes: {list(class_mapping.keys())}")
print("="*70)