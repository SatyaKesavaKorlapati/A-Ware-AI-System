...
class AWereEngine:
    ...
    def node_supervisor(self, state: AgentState) -> dict:
        ...
        - rag_specs_agent: Route here if the user asks about YOLO training metrics, precision, recall, synthetic dataset generation, VLM architecture, or system specs.
...
CRITICAL RULES:
        ...
        3. If you insert a NEW item, ALWAYS use a uniquely generated `prim_path` to avoid SQLite UNIQUE constraint errors! (e.g. use '/World/box_new_4819' or append a random number).
        4. If the user asks for a breakdown, distribution, or lists of multiple items across areas, ALWAYS use the `GROUP BY` clause (e.g. `GROUP BY rack_id`) instead of selecting raw rows. Returning thousands of raw rows will crash the system.
        5. If asked for exact locations of an item, select x, y, z, rack_id, physical_aisle, and shelf_level.
        6. Your final response should ONLY be the raw SQL query. No markdown formatting, no explanations.
...
if __name__ == "__main__":
    rewrite_engine()
    print("engine.py rewritten with contextualization node.")
